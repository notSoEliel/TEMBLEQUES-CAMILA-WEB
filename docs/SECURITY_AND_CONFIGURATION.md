...
`MCP_ADMIN_API_KEY` y `MCP_CLIENT_API_KEY` son credenciales externas con scopes separados. `MCP_BACKEND_ADMIN_TOKEN` y `MCP_BACKEND_CLIENT_TOKEN` son credenciales internas distintas para que el MCP llame a Hono. Todas las solicitudes HTTP a `/mcp`, incluidas `tools/list`, requieren Bearer cuando `MCP_AUTH_REQUIRED=true`.
