...
El repositorio incluye además `.github/workflows/mcp-staging-smoke.yml` para las historias H83-H100. Este workflow valida guest público, las 18 tools administrativas con API key de servicio, scopes, redacción de datos, operaciones administrativas y limpieza de fixtures. Si se configura `E2E_MCP_OAUTH_TOKEN`, valida también OAuth real y filtrado por rol. Requiere las secrets `MCP_REMOTE_URL`, `MCP_ADMIN_API_KEY`, `MCP_CLIENT_API_KEY`, `STAGING_BACKEND_URL`, `MCP_BACKEND_ADMIN_TOKEN` y `STAGING_FRONTEND_URL`. Nunca imprime sus valores.
